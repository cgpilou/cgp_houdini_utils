...
class Geometry(_generic.EditableGeometryItem):
    ...
    def createAttribute(self, geometryItemType, name, defaultValue):
        ...
        # init
        houType = {cgp_houdini_utils.constants.GeometryItemType.GEOMETRY: hou.attribType.Global,
                   ...
                   cgp_houdini_utils.constants.GeometryItemType.VERTEX: hou.attribType.Vertex}[geometryItemType]
        components = ((self,) if geometryItemType == cgp_houdini_utils.constants.GeometryItemType.GEOMETRY
                      else self.points() if geometryItemType == cgp_houdini_utils.constants.GeometryItemType.POINT
                      else self.primitives() if geometryItemType == cgp_houdini_utils.constants.GeometryItemType.PRIMITIVE
                      else self.vertices())

        # create (houdini only use the default value to get its type, the value wont be set)
        self.houGeometry().addAttrib(houType, name, defaultValue, False, False)

        # set default value
        if defaultValue:
            for component in components:
                component.attribute(name).setValue(defaultValue)
